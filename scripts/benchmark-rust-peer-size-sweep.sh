...
out_dir="${MICRORAPTOR_SIZE_SWEEP_OUT_DIR:-target/bench-results/rust-peer-size-sweep}"
sizes="${MICRORAPTOR_SIZE_SWEEP_RECORDS:-10000 50000 100000 500000 1000000}"
read_len="${MICRORAPTOR_SIZE_SWEEP_READ_LEN:-150}"
iters="${MICRORAPTOR_SIZE_SWEEP_ITERS:-3}"
consumer="${MICRORAPTOR_SIZE_SWEEP_CONSUMER:-light}"
compressions="${MICRORAPTOR_SIZE_SWEEP_COMPRESSIONS:-raw}"
base_project_dir="${MICRORAPTOR_SIZE_SWEEP_PROJECT_DIR:-target/rust-peer-size-sweep}"
summary="${out_dir}/summary.md"
tsv="${out_dir}/rust-peer-size-sweep.tsv"
...
tmp_tsv="$(mktemp "${out_dir}/rust-peer-size-sweep.raw.XXXXXX")"
...
for compression in ${compressions}; do
  for records in ${sizes}; do
    ...
    MICRORAPTOR_RUST_PEER_READ_LEN="${read_len}" \
    ...
    ' "${run_dir}/rust-library-peers.tsv" >> "${tmp_tsv}"
  done
done
...
awk -F '\t' '
  ...
  END {
    ...
    print "<text x=\"28\" y=\"32\" font-family=\"Arial, sans-serif\" font-size=\"20\" font-weight=\"700\">FASTQ parser framework size sweep</text>"
    print "<text x=\"28\" y=\"54\" font-family=\"Arial, sans-serif\" font-size=\"12\" fill=\"#555\">Best wall time over synthetic FASTQ inputs; lower is better</text>"
    ...
    for (t = 1; t <= tool_count; t++) {
      ...
      ly = top + 18 + (t - 1) * 22
      printf "<line x1=\"%d\" y1=\"%d\" x2=\"%d\" y2=\"%d\" stroke=\"%s\" stroke-width=\"3\"/>\n", left + plot_w - 210, ly - 4, left + plot_w - 186, ly - 4, color
      printf "<text x=\"%d\" y=\"%d\" font-family=\"Arial, sans-serif\" font-size=\"12\" fill=\"#222\">%s</text>\n", left + plot_w - 178, ly, name
    }

    print "<text x=\"" (left + plot_w / 2) "\" y=\"" (height - 25) "\" font-family=\"Arial, sans-serif\" font-size=\"13\" text-anchor=\"middle\" fill=\"#222\">Input size, resident FASTQ bytes</text>"
...
{
  printf '# Rust Peer Size Sweep\n\n'
  printf 'Generated by `%s`.\n\n' "$0"
  printf 'This sweep compares Rust FASTQ parser frameworks on deterministic synthetic FASTQ inputs of increasing size. Raw rows measure one resident byte buffer per size. Gzip rows measure one gzip-compressed resident byte buffer per size through the same streaming flate2 decoder. This is parser-framework evidence, not command-line workflow evidence for BGZF, trimming, or filtering.\n\n'
  ...
  printf '\nRaw data: [`rust-peer-size-sweep.tsv`](rust-peer-size-sweep.tsv)\n'
  printf 'Metadata: [`metadata.md`](metadata.md)\n'
} > "${summary}"

{
  printf '# Rust Peer Size Sweep Metadata\n\n'
